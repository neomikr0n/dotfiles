...
static int serving=0, operation_fd=-1;
static int port, remote=-1, rp=-1;
static int values[32], seen[32];
static long snapshot_time=0;
static long ms(void){struct timespec t; clock_gettime(CLOCK_MONOTONIC,&t);return t.tv_sec*1000+t.tv_nsec/1000000;}
static void die(const char *s){fprintf(stderr,"RME: %s\n",s);if(serving)longjmp(command_error,1);exit(1);}
static void send_bytes(unsigned char *b,int n){snd_seq_event_t e;snd_seq_ev_clear(&e);snd_seq_ev_set_source(&e,port);snd_seq_ev_set_dest(&e,remote,rp);snd_seq_ev_set_direct(&e);snd_seq_ev_set_sysex(&e,n,b);int rc=snd_seq_event_output_direct(seq,&e);if(rc<0){fprintf(stderr,"ALSA: %s\n",snd_strerror(rc));die("Error MIDI; no reintento de escritura.");}}
...
static int allowed_up(int target,int ref){return target<=-120 && target+(-50+60*ref)+60<=-50;}
...
static int run_command(int argc,char **argv){
 ...
 char lockpath[256];snprintf(lockpath,sizeof(lockpath),"/run/user/%u/rme-volume.lock",getuid());int fd=open(lockpath,O_CREAT|O_RDWR|O_NOFOLLOW,0600);operation_fd=fd;if(fd<0)die("No se puede crear bloqueo.");if(flock(fd,LOCK_EX|LOCK_NB)<0)return 0;
 ...
 if(port<0||snd_seq_connect_from(seq,port,remote,rp)<0||snd_seq_connect_to(seq,port,remote,rp)<0)die("No se puede suscribir a MIDI.");
 }
 /* Reuse only a recent hardware-confirmed snapshot in this live ALSA session.
  * Drain device updates first. Never use a disk cache or a disconnected state. */
 read_mode=0;read_for(2);read_mode=1;
 if(!snapshot_time || ms()-snapshot_time>750 || !strcmp(argv[1],"status") || !strncmp(argv[1],"check-",6)){query();snapshot_time=ms();}
 int v=values[12],ref=values[2],aut=values[3];printf("Line Out %.1f dB; Ref=%d Auto=%d Lock=%d Mute=%d Dim=%d\n",v/10.,ref,aut,values[13],values[15],values[16]);
 if(!strcmp(argv[1],"status")){return 0;}
 if(!strcmp(argv[1],"mute")||!strcmp(argv[1],"check-mute")){
  ...
  snapshot_time=ms();printf("RME Mute: %s\n",target?"ON":"OFF");return 0;
 }
 ...
 /* User-authorized ceiling: -12 dB, Ref +1 dBu RCA
  * (+7 dBu XLR), hence nominal unprocessed full-scale output <= -5 dBu.
 ...
 if(up){int ceiling=-50-(-50+60*ref)-60;if(ceiling>-120)ceiling=-120;if(v>=ceiling)die("Limite conservador alcanzado: no se sube.");if(target>ceiling)target=ceiling;}if(target < -1145)target=-1145;
 if(up && !allowed_up(target,ref))die("Limite conservador alcanzado: no se sube.");
 if(target==v){return 0;}
 if(check){printf("Simulacion: %.1f -> %.1f dB (sin escritura).\n",v/10.,target/10.);return 0;}
 ...
 read_mode=2;read_for(180);read_mode=1;if(!seen[12]||values[12]!=target)die("El RME no confirmo el cambio; no se reintenta.");snapshot_time=ms();printf("RME Line Out: %.1f dB\n",target/10.);return 0;
}

/* A short-lived, on-demand ALSA session. Requests still query actual hardware.
 * Client gate prevents a backlog of volume increases; no commands are replayed.
 */
static void serve(const char *path){
 int ls=socket(AF_UNIX,SOCK_STREAM|SOCK_CLOEXEC,0);
 struct sockaddr_un addr={.sun_family=AF_UNIX};snprintf(addr.sun_path,sizeof(addr.sun_path),"%s",path);
 unlink(path);umask(0077);
 if(ls<0||bind(ls,(void*)&addr,sizeof(addr))<0||listen(ls,1)<0)_exit(1);
 serving=1;long idle=ms();
 while(1){
  struct pollfd p={.fd=ls,.events=POLLIN};int ready=poll(&p,1,20);
  if(ready<0)break;
  if(!ready){
   if(ms()-idle>15000)break;
   if(seq){if(!setjmp(command_error)){read_mode=0;read_for(1);read_mode=1;}else{snd_seq_close(seq);seq=NULL;remote=-1;rp=-1;snapshot_time=0;}}
   continue;
  }
  idle=ms();
  int conn=accept4(ls,NULL,NULL,SOCK_CLOEXEC);if(conn<0)continue;
  struct timeval tv={.tv_sec=1};setsockopt(conn,SOL_SOCKET,SO_RCVTIMEO,&tv,sizeof(tv));
  char command[40]={0};ssize_t n=read(conn,command,sizeof(command)-1);
  if(n<=0){close(conn);continue;}command[strcspn(command,"\n")]=0;
  dup2(conn,STDOUT_FILENO);dup2(conn,STDERR_FILENO);read_mode=1;
  if(!setjmp(command_error)){char *args[]={"rme-volume",command,NULL};run_command(2,args);}
  else {if(seq){snd_seq_close(seq);seq=NULL;}remote=-1;rp=-1;snapshot_time=0;}
  fflush(stdout);fflush(stderr);
  if(operation_fd>=0){close(operation_fd);operation_fd=-1;}
  shutdown(conn,SHUT_RDWR);close(conn);
  int null=open("/dev/null",O_RDWR);dup2(null,1);dup2(null,2);close(null);
 }
 if(seq)snd_seq_close(seq);
 unlink(path);close(ls);_exit(0);
}
int main(int argc,char **argv){
 if(argc!=2||strlen(argv[1])>30){fprintf(stderr,"Uso: rme-volume accion\n");return 1;}
 char gatepath[128],path[108];snprintf(gatepath,sizeof(gatepath),"/run/user/%u/rme-volume-client.lock",getuid());
 snprintf(path,sizeof(path),"/run/user/%u/rme-volume-v2.sock",getuid());
 int gate=open(gatepath,O_CREAT|O_RDWR|O_NOFOLLOW,0600);if(gate<0)return 1;if(flock(gate,LOCK_EX|LOCK_NB)<0)return 0;
 struct sockaddr_un addr={.sun_family=AF_UNIX};snprintf(addr.sun_path,sizeof(addr.sun_path),"%s",path);
 int sock=socket(AF_UNIX,SOCK_STREAM|SOCK_CLOEXEC,0);
 if(connect(sock,(void*)&addr,sizeof(addr))<0){
  close(sock);pid_t child=fork();if(child<0)return 1;
  if(child==0){close(gate);setsid();if(fork()>0)_exit(0);for(int i=0;i<3;i++)close(i);int null=open("/dev/null",O_RDWR);dup2(null,0);dup2(null,1);dup2(null,2);if(null>2)close(null);serve(path);}
  waitpid(child,NULL,0);sock=-1;
  for(int i=0;i<50;i++){int s=socket(AF_UNIX,SOCK_STREAM|SOCK_CLOEXEC,0);if(connect(s,(void*)&addr,sizeof(addr))==0){sock=s;break;}close(s);usleep(10000);}
  if(sock<0){fprintf(stderr,"RME: no se pudo iniciar la sesion MIDI.\n");return 1;}
 }
 struct timeval tv={.tv_sec=2};setsockopt(sock,SOL_SOCKET,SO_RCVTIMEO,&tv,sizeof(tv));
 char cmd[40];int len=snprintf(cmd,sizeof(cmd),"%s\n",argv[1]);if(write(sock,cmd,len)!=len)return 1;
 char buf[4096];ssize_t n;int error=0;
 while((n=read(sock,buf,sizeof(buf)-1))>0){buf[n]=0;if(strstr(buf,"RME:")||strstr(buf,"ALSA:"))error=1;fwrite(buf,1,n,stdout);}
 if(n<0){fprintf(stderr,"RME: respuesta no confirmada; no se reintenta.\n");error=1;}
 close(sock);close(gate);return error;
}
