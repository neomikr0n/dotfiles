#!/usr/bin/env bash
# 🕹️ Modo Juego Automático Profesional Inteligente
# Solo activa Modo Juego en juegos reales
# Compatible con Arch Linux, Hyprland, Steam, Lutris

# ---------------- CONFIG ----------------
# Procesos de juegos “no-Steam”
...
# Detecta si hay un juego de Steam activo
steam_game_running() {
    # Listar procesos hijos de Steam
    for pid in $(pgrep -x steam); do
        # Extrae procesos que contienen /steamapps/common (ejecutable del juego)
        if pgrep -P "$pid" -a | grep -q "/steamapps/common"; then
            return 0
        fi
    done
    return 1
...
EOF
    ...
    if pgrep -x "mpvpaper" > /dev/null; then
        log "📌 Cerrando mpvpaper para liberar recursos..."
        pkill -x mpvpaper
    fi
}
...
while true; do
    ...
    # Detecta juegos de Steam reales
