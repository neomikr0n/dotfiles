#!/usr/bin/env bash
# 🕹️ Modo Juego para Hyprland - versión final estable
set -euo pipefail
...
# ────────────────────────────────────────────────
# 🔍 DETECCIÓN DE JUEGOS (refinada)
# ────────────────────────────────────────────────
is_game_running() {
    DEBUG_LOG "Verificando procesos activos..."

    # Buscar procesos relevantes y descartar Steam, Proton y otros
    local GAME_PROCESSES
    GAME_PROCESSES=$(pgrep -a -E "SteamAppId=|steam_app_|\.exe$" 2>/dev/null \
        | grep -viE "steam|proton|wineserver|bash" || true)
    ...
    # Fallback: detección por ventana activa en Hyprland
    if [[ "$XDG_SESSION_DESKTOP" == "Hyprland" ]] && hyprctl clients -j 2>/dev/null | grep -q "steam_app_"; then
...
# ────────────────────────────────────────────────
# 🔍 OBTENER PID DEL JUEGO (refinado)
# ────────────────────────────────────────────────
get_game_pid() {
    local pid
    pid=$(pgrep -a -E "SteamAppId=|steam_app_|\.exe$" 2>/dev/null \
        | grep -viE "steam|proton|wineserver|bash" \
        | awk '{print $1}' \
        | head -n1 || echo 0)
...
restore_mode() {
    ...
    # Restaurar Hyprland
    if [[ -f "$ML4W_FLAG" ]]; then
        DEBUG_LOG "🔁 Intentando recargar Hyprland..."
        if command -v hyprctl &>/dev/null; then
            # Forzar entorno Hyprland correcto
            export XDG_RUNTIME_DIR="/run/user/$(id -u)"
            export WAYLAND_DISPLAY="${WAYLAND_DISPLAY:-wayland-1}"
            hyprctl reload >/dev/null 2>&1 && DEBUG_LOG "✓ Hyprland recargado" || warn "No se pudo recargar Hyprland"
        else
            warn "hyprctl no disponible en PATH"
        fi
...
while true; do
    if is_game_running; then
        ...
        # Si Hyprland detecta juego pero pgrep no da PID, usamos un marcador simbólico
        [[ "$GAME_PID" == "0" || -z "$GAME_PID" ]] && GAME_PID="hyprland_game"

        if [[ "$CURRENT_GAME_PID" != "$GAME_PID" ]]; then
            activate_mode "$GAME_PID"
            CURRENT_GAME_PID="$GAME_PID"
            notify_modern "🎮 Gamemode ON" "Optimizaciones activadas"
        fi
    else
        if [[ "$CURRENT_GAME_PID" != "0" ]]; then
            restore_mode
            CURRENT_GAME_PID=0
            notify_modern "🎮 Gamemode OFF" "Optimizaciones desactivadas"
        fi
    fi
